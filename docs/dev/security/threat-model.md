...
| fork pull-request code with ledger secrets | unprivileged `pull_request` verifier-provenance preflight; full secret-backed ledger verification only on trusted `push` or explicit dispatch | pull requests do not receive authoritative ledger verification until trusted integration |
