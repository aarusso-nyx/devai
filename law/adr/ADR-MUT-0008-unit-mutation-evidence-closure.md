...
inspector_acceptance:
  ...
  - IA-003 -- Each retained member is reread through the certification-evidence sink, rehashed, and checked against an externally finalized unit receipt before prepare or export accepts it. Export retains byte-identical closure/receipt/member bytes in the provider-result; offline verification and evidence publication rehash and verify only that portable copy. No pathname, candidate callback, ambient store, package tarball, raw runner output, original host store, or live provider is a fallback.
