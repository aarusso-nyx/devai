---
id: ADR-REL-0019
title: Preserve and restate installed release policy binding
type: adr
status: accepted
date: 2026-09-03
authority: Architect
...
inspector_acceptance:
  - IA-006 -- The exact accepted ADR-REL-0017 bytes are preserved and digest-catalogued; altered bytes refuse, all required headings on this forward record validate, and the substantive source-resolution contract remains unchanged.
...
# Preserve and restate installed release policy binding

## Status

Accepted as a forward correction of the required-section defect in ADR-REL-0017.
That accepted record remains byte-for-byte unchanged. This record supersedes
it and restates its substantive source-resolution contract without relaxing
any binding, version, refusal, effect or historical-evidence rule. Later
independent pack/capacity corrections, including ADR-REL-0018, are unaffected.
...
The accepted record used `## Consequences and implementation map`, which is
not the exact `Consequences` heading required by the frozen ADR classifier.
That is a reference defect, not a reason to weaken the sensor or rewrite
accepted history. The existing `preserved-invalid-accepted-record` catalog
mechanism already handles this exact class of historical defect.
...
### Preserve accepted history and restate forward

ADR-validation policy `2.7.0` adds exactly one exception entry for
`ADR-REL-0017-package-owned-release-policy-resolution.md`, with raw SHA-256
`07f1d17f88d8c48f25b51cff7e882cb2c7c23a3d529098c51b8c19431a946e19`. Its disposition is
`preserved-invalid-accepted-record`, source format
`v2-record-missing-required-section`, and complete catalog metadata is copied
from the accepted record. The complete path-sorted catalog and its schema
example carry the recomputed JCS digest. The semantic resolver's legacy
allowlist adds the exact same reference/path/disposition pair, preserving its
mandatory bijection with materialized catalog records; the schema example
mirrors the complete allowlist. The allowlist is outside the catalog digest's
defined entries-array preimage. No wildcard, general exception,
classifier relaxation or in-place repair is introduced. Any changed byte in
the preserved record refuses. Supersession resolves its substantive subjects
to this forward record; the following source-resolution rules are unchanged.
...
ADR-validation policy adds only the one exact historical-record exception;
the policy schema changes only its corresponding example. Existing classifier
