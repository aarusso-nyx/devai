...
export function createReleaseCertificationProvider(
  ...
  const provider: ReleaseProvider = async (requestInput) => {
    try {
      ...
      // The code below is closed so the attempt record stays schema-valid, but on its own
      // that discards the only account of what went wrong: a native RangeError or
      // TypeError reaches the operator as release-certification-generated-output-untrusted
      // with no cause, and recovering it costs a rebuild and a full certify run. The
      // diagnostic stream is not the ledger, so the original goes there unabridged.
      process.stderr.write(
        `release certify: cause: ${
      ...
      return {
        ...
        code:
          error instanceof Error && /^(?:release|rpl)-[a-z0-9-]+$/u.test(error.message)
            ? error.message
