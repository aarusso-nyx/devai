...
// It exits explicitly once both channels are sent and its invariants hold. The exit
// status reports whether this program delivered its observation and raw report, never
// whether mutants survived: the host decides that from the report it is handed. Vitest
// runs in-process under Stryker's runner and sets process.exitCode on a failing test,
// and under mutation testing every killed mutant is a failing test, so an implicit exit
// reports a complete run as an abnormal worker and discards the evidence it just
// produced. Both sends are synchronous writeSync loops, so nothing is buffered here.
