...
describe('retired protected mutation execution', () => {
  it('rejects mutation programs before any Docker operation', () => {
    const value = mutationFixture(envelope());
    try {
      const before = dockerCalls.length;
      expect(() => invokeMutation(value)).toThrow('mutation-offloaded-to-bedel');
