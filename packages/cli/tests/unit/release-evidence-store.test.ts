...
function rebindStoredOutputReceipt(value: StoredCertificationOutput): void {
  const handle = value.output_blob_handle as unknown as Parameters<
...
describe('durable external certification evidence store', () => {
  ...
  it.each([
    [
      'an unsafe path',
      (value: StoredCertificationOutput): unknown => (value.path = '../report.json'),
    ],
    ['an unsupported mode', (value: StoredCertificationOutput): unknown => (value.mode = '100600')],
    [
      'a malformed digest',
      (value: StoredCertificationOutput): void => {
  ...
  it('retains and reopens a zero-byte generated output', async () => {
    ...
    if (!receipt) throw new Error('fixture output receipt missing');
