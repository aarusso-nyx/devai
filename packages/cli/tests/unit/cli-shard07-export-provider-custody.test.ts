...
const mocks = vi.hoisted(() => ({
  ...
  transcriptBeforeSigning: undefined as Buffer | undefined,
...
vi.mock('../../src/services/release-export-mutation-evidence.js', () => ({
  createReleaseExportMutationEvidence: (...args: unknown[]) => {
    ...
    return (
      mocks.mutationGate?.then(() => Buffer.from('mutation-carrier')) ??
      Buffer.from('mutation-carrier')
    );
  },
  ...
  return {
    ...
    createReleaseExportArtifactStore: async (...args: unknown[]) => {
      ...
      const transaction = {
        ...
        readTranscript: async () => Buffer.from(mocks.transcriptBeforeSigning ?? transcript),
...
describe('release export provider depth', () => {
  ...
  it('exports exact closures and provider results, then commits and switches readers', async () => {
    ...
    expect(providerResults).toEqual([
      ...
      expect.objectContaining({
        ...
        certification_evidence: expect.objectContaining({
          ...
          sha256: 'c'.repeat(64),
          size_bytes: 21,
          bytes_base64: 'Y2VydGlmaWNhdGlvbg==',
        }),
      }),
    ]);
    ...
    expect(
      material.release_units[0]?.packages.map((pkg) => [
        pkg.package_id,
        (pkg.evidence_manifest as { opaque_handle?: string } | undefined)?.opaque_handle,
        (pkg.provider_result as { opaque_handle?: string } | undefined)?.opaque_handle,
      ]),
    ).toEqual([
  ...
  it('preserves absence of optional plan and evidence readers', async () => {
    ...
    expect(Object.keys(mutationInput.plan)).toEqual([]);
    expect(mutationInput.source).toEqual({ unit_mutation_maximum_bytes: 4096 });
    const certificationInput = mocks.createCertification.mock.calls[0]![0] as any;
    expect(certificationInput.source).toEqual({});
    expect(Object.keys(certificationInput.source)).toEqual([]);
  });

  it('accepts a wholly absent optional certification source without dereferencing it', async () => {
    resetPrepared();
    const base = options() as any;
    const { provider } = createReleaseExportProvider({ ...base, certification_source: undefined });
    await expect(provider(request(), context())).resolves.toMatchObject({ outcome: 'success' });
    expect(mocks.createCertification.mock.calls[0]![0].source).toEqual({});
  });

  it('releases one acquired invocation after a pre-sign failure so a later attempt can proceed', async () => {
    resetPrepared();
    mocks.mutationError = new Error('source failed');
    const created = createReleaseExportProvider(options());
    await expect(created.provider(request(), context())).resolves.toMatchObject({
      outcome: 'failure',
    });
    mocks.mutationError = undefined;
    await expect(created.provider(request(), context())).resolves.toMatchObject({
  ...
  it('does not let a rejected concurrent caller release the active invocation', async () => {
    resetPrepared();
    let release!: () => void;
    mocks.mutationGate = new Promise<void>((resolve) => {
      release = resolve;
    });
    const created = createReleaseExportProvider(options());
    const first = created.provider(request(), context());
    await vi.waitFor(() => expect(mocks.createMutation).toHaveBeenCalledTimes(1));
    await expect(created.provider(request(), context())).resolves.toMatchObject({
      outcome: 'failure',
    });
    const third = created.provider(request(), context());
    setTimeout(release, 25);
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(mocks.createMutation).toHaveBeenCalledTimes(1);
    await expect(first).resolves.toMatchObject({ outcome: 'success' });
    await expect(third).resolves.toMatchObject({ outcome: 'failure' });
  });
  ...
  it('rejects a valid JSON transcript whose bytes are not the canonical encoding', async () => {
    resetPrepared();
    const noncanonical = Buffer.from('{"kind":"fixture-export-transcript", "extra":true}');
    mocks.transcriptBeforeSigning = noncanonical;
    mocks.transcriptAfterSigning = noncanonical;
    const created = createReleaseExportProvider(options());
    await expect(created.provider(request(), context())).resolves.toMatchObject({
      outcome: 'unknown',
      code: UNKNOWN,
  ...
  it('retains hashing, ordering, equality, and refusal behavior in a freshly evaluated module', async () => {
    vi.resetModules();
    const fresh = await import('../../src/services/release-export-provider.js');
    ...
    const result = await created.provider(request(), context());
    expect(result.outcome).toBe('success');
    expect(result.material?.artifacts.map((row: { kind: string }) => row.kind)).toEqual([
      'evidence-manifest',
      'evidence-manifest',
      'package-manifest',
      'provider-result',
