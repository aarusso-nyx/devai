...
describe('release certified evidence carrier', () => {
  it('round-trips multi-megabyte result documents without recursive base64 matching', () => {
    ...
    const largeResults = [large, ...results.slice(1)];
    const boundReceipt = {
      ...receipt,
      tasks: largeResults.map((value) => ({
        nodeId: value.nodeId,
        taskKey: value.taskKey,
        resultDigest: digest(value),
      })),
    ...
    const bytes = build({
      maximum_bytes: maximum,
      task_results: largeResults,
      candidate_receipt: boundReceipt,
    });
    const decoded = readCertifiedEvidenceCarrier(bytes, maximum);
    expect(decoded.task_results.some((value) => value.length > 6 * 1024 * 1024)).toBe(true);
    expect(() => readCertifiedEvidenceCarrier(bytes, 1024)).toThrow(INVALID);
  });
