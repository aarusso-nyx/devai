...
// buildx from v0.10 attaches provenance and SBOM attestations by default, and those
// carry a build timestamp and a random ref, so the image identity differs on every
// build. Disabling them by environment rather than by --provenance=false keeps this
// working on older buildx, which does not know the flag but ignores the variable.
const environment = {
  PATH: '/usr/bin:/bin',
  HOME: '/tmp',
  LANG: 'C',
