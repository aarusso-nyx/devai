...
it('activates static mutants before imports while retaining the explicit test-file population', () => {
  ...
  try {
    ...
    writeFileSync(
      join(root, 'value.js'),
      `export const REQUIRED = 'must-match';
...
`;
    for (const name of ['value.test.js', 'other.test.js'])
      writeFileSync(join(root, 'tests', name), test);
    writeFileSync(
    ...
    writeFileSync(
      ...
      JSON.stringify({
        mutate: ['value.js'],
        testFiles: ['tests/value.test.js', 'tests/other.test.js'],
    ...
    expect(result.status, `Retained fixture: ${root}`).toBe(0);
    const report = JSON.parse(readFileSync(join(root, 'report.json'), 'utf8')) as {
      files: Record<string, { mutants: { static?: boolean; status: string }[] }>;
